...
class BillingManager:
    ...
    def update_prices(self):
        ...
        for vps in self.vps_data:
            # 实时计算使用时长，精确到分钟
            usage_result = self.calculate_usage_period(vps)
            if isinstance(usage_result, tuple) and len(usage_result) == 4:
                usage_string, days, hours, minutes = usage_result
                ...
                if isinstance(usage_result, tuple) and len(usage_result) == 4:
                    ...
                    if days > 0 or hours > 0 or minutes > 0:
                        ...
                        bill_row = {
                            ...
                            '使用状态': vps.get('status', ''),
                            '销毁时间': vps.get('cancel_date', '') if vps.get('status') == "销毁" else '',
...
if __name__ == "__main__":
    ...
    parser.add_argument('--action', type=str, required=True, 
                        help='要执行的操作: get_current_month_bill, get_monthly_bill, get_monthly_bill_summary, save_monthly_billing_to_excel, get_all_vps, save_vps, delete_vps, init_sample_data, update_prices')
    ...
    try:
        ...
        if args.action == 'get_current_month_bill':
            ...
            if success:
                print(f"成功删除VPS: {args.vps_name}")
                sys.exit(0)
            else:
                print(f"删除VPS失败: {args.vps_name}", file=sys.stderr)
                sys.exit(1)
            ...
            if success:
                ...
                print("初始化示例数据失败", file=sys.stderr)
                sys.exit(1)
            ...
            if success:
                print("成功更新VPS价格和使用时长")
                sys.exit(0)
            else:
                print("更新VPS价格和使用时长失败", file=sys.stderr)
                sys.exit(1)
        ...
        # 输出错误信息到标准错误
        print(f"执行操作时出错: {str(e)}", file=sys.stderr)
        sys.exit(1) 